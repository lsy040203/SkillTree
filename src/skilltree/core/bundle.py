...
def build_bundle(repository_root: Path) -> dict[str, Any]:
    """Build release artifacts in a temporary directory then publish them."""
    ...
    if plugin_manifest.get("name") != "skilltree" or not isinstance(plugin_version, str) or _base_version(plugin_version) != version:
    ...
    with tempfile.TemporaryDirectory(prefix="skilltree-bundle-") as temp_dir:
        ...
        _discover_migrations(plugin_root)
        ...
        manifest = _make_manifest(plugin_root, plugin_version, version, wheel.name)
...
def validate_bundle(plugin_root: Path) -> dict[str, Any]:
    """Validate all offline artifacts without installing or executing them."""
...
def _make_manifest(plugin_root: Path, plugin_version: str, core_version: str, wheel_filename: str) -> dict[str, Any]:
    wheel_path = Path("runtime") / "wheels" / wheel_filename
    migrations = _discover_migrations(plugin_root)
    runtime_paths = sorted(
        [
            "runtime/skilltree_bootstrap.ps1",
            "runtime/bundle_validate.py",
    ...
    manifest: dict[str, Any] = {
        ...
        "schema": {"version": SCHEMA_VERSION, "migration_version": len(migrations)},
        "migrations": [
            {"version": version, "path": path, "sha256": _sha256(plugin_root / path)}
        ...
        "wheels": [{"filename": wheel_filename, "distribution": CORE_DISTRIBUTION, "version": core_version, "sha256": _sha256(plugin_root / wheel_path)}],
        "hook_bundle": {
...
def _validate_plugin_and_core(plugin_root: Path, manifest: dict[str, Any]) -> None:
    ...
    plugin_manifest_path = _validate_file_entry(plugin_root, plugin, "manifest_path")
    plugin_manifest = _read_json(plugin_manifest_path)
    if (
        not isinstance(plugin["version"], str)
        or _base_version(plugin["version"]) != core["version"]
        or plugin_manifest.get("name") != plugin["name"]
        or plugin_manifest.get("version") != plugin["version"]
        or schema.get("version") != SCHEMA_VERSION
        or not isinstance(schema.get("migration_version"), int)
        or schema["migration_version"] < 1
    ):
...
def _base_version(version: str) -> str:
    """Return the semantic version portion before a local cachebuster suffix."""
...
def _validate_migrations(plugin_root: Path, manifest: dict[str, Any]) -> None:
    ...
    actual_migrations = _discover_migrations(plugin_root)
    actual_paths = {path for _, path in actual_migrations}
    if actual_paths != found_paths:
        raise BundleValidationError("migration directory does not match manifest")


def _discover_migrations(plugin_root: Path) -> list[tuple[int, str]]:
    migration_directory = plugin_root / "migrations"
    if not migration_directory.is_dir():
        raise BundleValidationError("migration directory is missing")
    discovered: list[tuple[int, str]] = []
    for path in migration_directory.glob("*.sql"):
        relative_path = path.relative_to(plugin_root).as_posix()
        match = _MIGRATION_PATTERN.fullmatch(relative_path)
        if not match:
            raise BundleValidationError("invalid migration path")
        discovered.append((int(match.group(1)), relative_path))
    discovered.sort()
    if not discovered or [version for version, _ in discovered] != list(range(1, len(discovered) + 1)):
        raise BundleValidationError("migration sequence mismatch")
    return discovered
...
def _validate_hook_bundle(plugin_root: Path, manifest: dict[str, Any]) -> None:
    ...
    if files != sorted(files) or files != [
        "hooks/hooks.json",
        "runtime/skilltree_bootstrap.ps1",
        "runtime/skilltree_hook.py",
        "scripts/invoke-hook.ps1",
...
def _build_core_wheel(repository_root: Path, wheel_dir: Path) -> None:
    ...
    # Store members without compression so wheel bytes do not depend on the
    # zlib implementation bundled with the builder's Python interpreter.
    with ZipFile(wheel_path, "w", compression=ZIP_STORED) as wheel:
...
def _write_zip_member(wheel: ZipFile, archive_path: str, contents: bytes) -> None:
    ...
    wheel.writestr(member, contents, compress_type=ZIP_STORED)
