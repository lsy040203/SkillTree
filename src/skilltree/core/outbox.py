...
class AtomicOutbox:
    ...
    def enqueue_trace_event(self, event: dict[str, object]) -> Path:
        """Atomically enqueue one validated TraceEvent for the single writer."""
        validated = build_trace_event(**event)  # type: ignore[arg-type]
        return self.enqueue(validated)
