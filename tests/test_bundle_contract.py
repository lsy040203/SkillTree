...
from skilltree import __version__


class BundleContractTests(unittest.TestCase):
    def test_core_package_version_matches_the_current_release_contract(self) -> None:
        self.assertEqual(__version__, "0.4.1")

    def test_current_bundle_has_a_complete_verified_offline_manifest(self) -> None:
        ...
        self.assertEqual(manifest["plugin"]["version"].split("+", 1)[0], "0.4.1")
        self.assertEqual(manifest["core"]["distribution"], "skilltree-core")
        self.assertEqual(manifest["core"]["version"], "0.4.1")
        self.assertEqual(manifest["schema"], {"version": "skilltree/v1", "migration_version": 9})
        self.assertEqual(
            manifest["migrations"],
            [
                {
                ...
                {
                    "version": 2,
                    "path": "migrations/0002_p1_registry.sql",
                    "sha256": manifest["migrations"][1]["sha256"],
                },
                {"version": 3, "path": "migrations/0003_p2_routing.sql", "sha256": manifest["migrations"][2]["sha256"]},
                {"version": 4, "path": "migrations/0004_p3_turn_binding.sql", "sha256": manifest["migrations"][3]["sha256"]},
                {"version": 5, "path": "migrations/0005_p3_trace.sql", "sha256": manifest["migrations"][4]["sha256"]},
                {"version": 6, "path": "migrations/0006_p4_learning.sql", "sha256": manifest["migrations"][5]["sha256"]},
                {"version": 7, "path": "migrations/0007_p5_memory.sql", "sha256": manifest["migrations"][6]["sha256"]},
                {"version": 8, "path": "migrations/0008_p6_replay.sql", "sha256": manifest["migrations"][7]["sha256"]},
                {"version": 9, "path": "migrations/0009_p66_extensions.sql", "sha256": manifest["migrations"][8]["sha256"]},
            ],
        )
        self.assertTrue((PLUGIN_ROOT / "runtime" / "wheels" / "skilltree_core-0.4.1-py3-none-any.whl").is_file())
    ...
    def test_hook_bundle_covers_the_windows_hook_launcher(self) -> None:
        manifest = validate_bundle(PLUGIN_ROOT)

        self.assertIn("scripts/invoke-hook.ps1", manifest["hook_bundle"]["files"])

    def test_p1_migration_adds_only_the_registry_table(self) -> None:
        sql = (PLUGIN_ROOT / "migrations" / "0001_p0_runtime.sql").read_text(encoding="utf-8")
        registry_sql = (PLUGIN_ROOT / "migrations" / "0002_p1_registry.sql").read_text(encoding="utf-8")
        ...
        self.assertIn("CREATE TABLE skills", registry_sql)
        self.assertNotIn("CREATE TABLE runtime_config", registry_sql)
        self.assertNotIn("CREATE TABLE audit_events", registry_sql)
        self.assertNotIn("CREATE TABLE schema_migrations", registry_sql)

    def test_hook_does_not_read_the_retired_config_file(self) -> None:
    ...
    def test_local_cachebuster_keeps_plugin_suffix_but_uses_base_core_version(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            copied_root = Path(temp_dir) / "repository"
            _copy_repository_for_build(ROOT, copied_root)
            plugin_json = copied_root / "plugins" / "skilltree" / ".codex-plugin" / "plugin.json"
            plugin_manifest = json.loads(plugin_json.read_text(encoding="utf-8"))
            plugin_manifest["version"] = "0.4.1+codex.test"
            plugin_json.write_text(json.dumps(plugin_manifest, indent=2) + "\n", encoding="utf-8")

            manifest = build_bundle(copied_root)
            validated = validate_bundle(copied_root / "plugins" / "skilltree")

        self.assertEqual(manifest["plugin"]["version"], "0.4.1+codex.test")
        self.assertEqual(manifest["core"]["version"], "0.4.1")
        self.assertEqual(validated["plugin"]["version"], "0.4.1+codex.test")

    def test_cachebuster_with_different_base_version_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            copied_root = Path(temp_dir) / "repository"
            _copy_repository_for_build(ROOT, copied_root)
            plugin_json = copied_root / "plugins" / "skilltree" / ".codex-plugin" / "plugin.json"
            plugin_manifest = json.loads(plugin_json.read_text(encoding="utf-8"))
            plugin_manifest["version"] = "0.4.2+codex.test"
            plugin_json.write_text(json.dumps(plugin_manifest, indent=2) + "\n", encoding="utf-8")

            with self.assertRaisesRegex(ValueError, "plugin/core version mismatch"):
                build_bundle(copied_root)
    ...
    def test_core_wheel_build_does_not_invoke_pip_or_a_pep517_backend(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(wheel_path) as wheel:
                ...
                metadata = wheel.read("skilltree_core-0.4.1.dist-info/METADATA").decode("utf-8")
                wheel_metadata = wheel.read("skilltree_core-0.4.1.dist-info/WHEEL").decode("utf-8")

        self.assertIn("skilltree/bundle.py", names)
        self.assertIn("skilltree_core-0.4.1.dist-info/RECORD", names)
    ...
    @unittest.skipUnless(os.environ.get("SKILLTREE_UV_PYTHON"), "requires an explicit uv Python path")
    def test_bundle_bytes_are_stable_across_supported_builder_interpreters(self) -> None:
        uv_python = os.environ["SKILLTREE_UV_PYTHON"]
        manifests: list[dict[str, object]] = []
        for builder in (sys.executable, uv_python):
            with tempfile.TemporaryDirectory() as temp_dir:
                copied_root = Path(temp_dir) / "repository"
                _copy_repository_for_build(ROOT, copied_root)
                completed = subprocess.run(
                    [builder, "tools/build_bundle.py"],
                    cwd=copied_root,
                    capture_output=True,
                    text=True,
                    check=False,
                ...
                self.assertEqual(completed.returncode, 0, completed.stderr)
                manifest_path = copied_root / "plugins" / "skilltree" / "runtime" / "bundle-manifest.json"
                manifests.append(json.loads(manifest_path.read_text(encoding="utf-8")))

        self.assertEqual(manifests[0]["bundle_hash"], manifests[1]["bundle_hash"])
        self.assertEqual(manifests[0]["core"], manifests[1]["core"])
